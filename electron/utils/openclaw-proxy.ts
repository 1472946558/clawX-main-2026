...
interface SyncProxyOptions {
   ...
   * currently disabled in canvasland settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync canvasland global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because canvasland proxy is disabled and preserve mode is enabled');
      return;
    }
