...
# Post-installation script for canvasland on Linux
...
# Create symbolic link for canvasland app binary
if [ -x /opt/canvasland/clawx ]; then
    ln -sf /opt/canvasland/clawx /usr/local/bin/clawx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/canvasland/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/canvasland/chrome-sandbox' || true
else
    chmod 0755 '/opt/canvasland/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/canvasland/resources/apparmor-profile'
...
echo "canvasland has been installed successfully."
