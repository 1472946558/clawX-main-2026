...
test.describe('canvasland Electron smoke flows', () => {
  test('shows the setup wizard on a fresh profile', async ({ page }) => {
    ...
    await expect(page.getByTestId('setup-agent-name-input')).toBeVisible();
    await expect(page.getByTestId('setup-agent-persona-textarea')).toBeVisible();
