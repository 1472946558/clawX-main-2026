...
describe('host services', () => {
  ...
  it('updates the main agent profile and schedules gateway reload', async () => {
    ...
    updateAgentProfileMock.mockResolvedValue(snapshot);
    const gatewayManager = {
      getStatus: vi.fn(() => ({ state: 'running' })),
      debouncedReload: vi.fn(),
    };
    ...
    await expect(createAgentsApi({ gatewayManager: gatewayManager as never }).updateProfile({
      id: 'main',
      name: 'Helper',
      persona: 'Calm and precise.',
    })).resolves.toEqual({ success: true, ...snapshot });

    expect(updateAgentProfileMock).toHaveBeenCalledWith('main', {
      name: 'Helper',
      persona: 'Calm and precise.',
    });
    expect(gatewayManager.debouncedReload).toHaveBeenCalledTimes(1);
