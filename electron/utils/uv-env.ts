...
function buildcanvaslandUvConfigToml(): string {
  return [
    '# This file is managed by canvasland.',
    '# It lets uv use canvasland-selected mirrors even when host exec filters UV_INDEX_URL.',
...
export function getcanvaslandUvConfigFilePath(): string {
  return path.join(getOpenClawConfigDir(), 'clawx', 'uv.toml');
}

function ensurecanvaslandUvConfigFile(): string | null {
  const filePath = getcanvaslandUvConfigFilePath();
  const content = buildcanvaslandUvConfigToml();
  ...
  try {
    ...
    logger.warn('Failed to write canvasland uv config file:', err);
    return null;
  }
}
...
export async function getUvMirrorEnv(): Promise<Record<string, string>> {
  ...
  const uvConfigFile = ensurecanvaslandUvConfigFile();
  return uvConfigFile
