...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜclawxなのか">なぜcanvaslandなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**canvasland**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、canvaslandはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

canvaslandはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。canvaslandはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**

| 課題 | canvaslandのソリューション |
|------|----------------------|
...
canvaslandは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。

私たちはアップストリームのOpenClawプロジェクトとの厳密な整合性を維持することにコミットしており、公式リリースが提供する最新の機能、安定性の改善、エコシステムの互換性に常にアクセスできることを保証します。

開発者モードを有効にすると、サイドバーにはネイティブの Dreams ページも表示され、canvasland 内で OpenClaw の記憶レビュー、夢日記、基本メンテナンス操作を扱えます。詳細な診断が必要な場合は、そのページから完全版の OpenClaw Dreams UI も開けます。
...
`@agent` で別のエージェントを選ぶと、canvasland はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
canvasland には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
canvasland はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。
...
### 🔐 隠れた上流モデルレイヤー
canvasland は一般ユーザー向けの独立したモデルページを表示しません。ユーザーは AI アシスタント作成時に GPT 5.4、GPT 5.5、Qwen 3.6 Plus、Qwen 3.7 Max から選択し、ウォレットで残高とポイント消費履歴を確認できます。New API へのアクセス、キーのコピー、Provider/Base URL の設定は不要です。
実際の New API キーは canvasland のウォレット／モデルプロキシのサーバー環境変数にのみ保存します。高度な Provider 設定は開発者／管理者向け設定に残します。
...
商品ショート動画ワークベンチは現在の Provider を使用し、そのモデル一覧から動画モデルを優先表示します。商品テキスト、主要訴求、対象プラットフォーム、画面比率を入力でき、非同期タスクではローカルジョブ ID、Provider taskId、正規化された状態、秘匿化済みレスポンス概要、手動状態確認を表示し、完了後は動画プレーヤーと結果 URL を表示します。
AI Apps の料金はサーバー管理のポイントプランです。文案生成は 10〜30 ポイント、画像生成は 30 または 60 ポイント、動画生成は 300〜600 ポイントです。画面には通貨換算を出さず、Pro、優先生成、透かしなし、画質、長さなどの特典とポイントだけを表示します。成功時のみリクエスト ID 単位で一度だけ消費し、失敗時は消費しません。チャージプランには、5,000 基本ポイント＋1,000 ボーナスポイントなどの特典があります。
OpenAI-compatible ゲートウェイを **Custom プロバイダー** で使う場合、開発者／管理者モードの **設定 → AI Providers → Provider 編集** でカスタム `User-Agent` を設定でき、互換性が必要なエンドポイントで有効です。
互換ゲートウェイで `/models` が認証以外の理由で使えない場合、canvasland は API キー検証時に軽量な `/chat/completions` または `/responses` プローブへ自動フォールバックします。

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。canvaslandはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に canvasland が自動的に起動します。

### 🔔 更新通知
canvasland は起動時に新しいバージョンを自動確認できます。更新が見つかるとアプリ内通知を表示し、ダウンロードやインストールはユーザーが選択した後にのみ実行されます。
...
[Releases](https://github.com/ValueCell-ai/canvasland/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
canvaslandを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：

1. **Agent 人格** – メイン Agent の名前、人格、優先言語を設定
2. **環境チェック** – 同梱 OpenClaw ランタイムと Gateway を検証
3. **必須コンポーネント** – アシスタントに必要なローカルツールをインストール
4. **完了** – メイン画面に入り、必要に応じて Provider、スキル、チャンネルを設定

サポート対象のシステム言語がある場合、ウィザードはその言語を初期選択し、未対応の場合は英語にフォールバックします。

### プロキシ設定

canvaslandには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、canvaslandは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- canvaslandはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
- canvaslandのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
canvaslandは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：

```
┌────────────────────────────────────────────────────────────────────┐
│                        canvasland デスクトップアプリ                       │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │              Electron メインプロセス                            │  │
...
- canvasland は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit canvasland** を使用してください。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がcanvaslandをより良くするのに役立ちます。
...
canvaslandは優れたオープンソースプロジェクトの上に構築されています：
...
### canvasland パートナープログラム 🚀

canvasland パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に canvasland を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、canvasland チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/canvasland&type=Date" alt="スター履歴チャート" />
</p>
...
canvaslandは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
