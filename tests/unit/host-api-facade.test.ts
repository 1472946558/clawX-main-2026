...
describe('hostApi facade', () => {
  ...
  it('calls agents.list through hostInvoke', async () => {
    hostInvoke
      .mockResolvedValueOnce({ id: 'req-1', ok: true, data: { success: true, agents: [] } })
      .mockResolvedValueOnce({ id: 'req-2', ok: true, data: { success: true, agents: [{ id: 'main' }] } });
    const { hostApi } = await import('@/lib/host-api');

    await expect(hostApi.agents.list()).resolves.toEqual({ success: true, agents: [] });
    await expect(hostApi.agents.updateProfile('main', {
      name: 'Helper',
      persona: 'Calm and precise.',
    })).resolves.toEqual({ success: true, agents: [{ id: 'main' }] });
    expect(hostInvoke).toHaveBeenNthCalledWith(1, expect.objectContaining({
      module: 'agents',
      action: 'list',
    }));
