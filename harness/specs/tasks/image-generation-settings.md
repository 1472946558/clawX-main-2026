...
expectedUserBehavior:
  ...
  - Saving the OpenAI-compatible image endpoint writes a canvasland-owned provider (`clawx-openai-image`) and auth profile, enables `request.allowPrivateNetwork` for trusted custom endpoints, and leaves `models.providers.openai` untouched so chat continues to use the regular OpenAI provider.
...
`~/.openclaw/agents/{id}/agent/auth-profiles.json`. canvasland's OpenAI-compatible
...
canvasland syncs chat defaults on provider switch, but image generation is configured independently from its developer-only Image Generation page and is never auto-synced from the default chat provider.
