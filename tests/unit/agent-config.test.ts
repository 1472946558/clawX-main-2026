...
describe('agent config lifecycle', () => {
  ...
  it('seeds a default canvasland IDENTITY.md for newly created agent workspaces', async () => {
    await writeOpenClawJson({
      agents: {
        list: [{ id: 'main', name: 'Main', default: true }],
      },
  ...
  it('updates the main agent profile and writes persona bootstrap files', async () => {
    ...
    await writeFile(join(workspaceDir, 'SOUL.md'), '# Existing soul\n\nKeep this paragraph.', 'utf8');

    const { updateAgentProfile } = await import('@electron/utils/agent-config');

    const snapshot = await updateAgentProfile('main', {
      name: 'Navigator',
      persona: 'Calm, direct, and careful with code.',
    });

    expect(snapshot.agents.find((agent) => agent.id === 'main')?.name).toBe('Navigator');
    const config = await readOpenClawJson();
    expect((config.agents as { list: Array<{ id: string; name?: string }> }).list[0].name).toBe('Navigator');

    const soul = await readFile(join(workspaceDir, 'SOUL.md'), 'utf8');
    const identity = await readFile(join(workspaceDir, 'IDENTITY.md'), 'utf8');
    expect(soul).toContain('Keep this paragraph.');
    expect(soul).toContain('<!-- clawx:agent-profile:begin -->');
    expect(soul).toContain('- **Name:** Navigator');
    expect(soul).toContain('Calm, direct, and careful with code.');
    expect(identity).toContain('- **Name:** Navigator');
    expect(identity).toContain('Calm, direct, and careful with code.');
  });
});
