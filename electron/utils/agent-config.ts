...
const AGENT_PROFILE_BEGIN = '<!-- clawx:agent-profile:begin -->';
const AGENT_PROFILE_END = '<!-- clawx:agent-profile:end -->';
...
export function buildAgentProfileSection(name: string, persona: string): string {
  const normalizedName = normalizeAgentName(name);
  const normalizedPersona = normalizeAgentPersona(persona);
  return [
    '## canvasland Agent Profile',
    '',
    `- **Name:** ${normalizedName}`,
    '- **Role:** primary desktop AI agent',
    '',
    '### Persona',
    normalizedPersona || 'Act as a concise, capable, and practical AI assistant.',
    '',
  ].join('\n');
}

function mergeAgentProfileSection(existing: string, section: string): string {
  const wrapped = `${AGENT_PROFILE_BEGIN}\n${section.trim()}\n${AGENT_PROFILE_END}`;
  const beginIdx = existing.indexOf(AGENT_PROFILE_BEGIN);
  const endIdx = existing.indexOf(AGENT_PROFILE_END);
  if (beginIdx !== -1 && endIdx !== -1 && endIdx > beginIdx) {
    return `${existing.slice(0, beginIdx)}${wrapped}${existing.slice(endIdx + AGENT_PROFILE_END.length)}`;
  }
...
async function provisionAgentFilesystem(
  ...
  // on first use, but canvasland still pre-seeds IDENTITY.md so desktop workspaces
...
export async function updateAgentProfile(
  ...
  return withConfigLock(async () => {
    ...
    const normalizedPersona = normalizeAgentPersona(profile.persona);
    const index = entries.findIndex((entry) => entry.id === agentId);
    if (index === -1) {
      throw new Error(`Agent "${agentId}" not found`);
    }
    ...
    const workspaceDir = expandPath(
      ...
      || (entries[index].id === MAIN_AGENT_ID ? getDefaultWorkspacePath(config) : `~/.openclaw/workspace-${entries[index].id}`),
    );
    await ensureDir(workspaceDir);

    const section = buildAgentProfileSection(normalizedName, normalizedPersona);
    const profileFiles = ['SOUL.md', 'IDENTITY.md'];
    for (const fileName of profileFiles) {
