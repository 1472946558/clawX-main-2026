...
let cachedAutoUpdater: ElectronUpdater | null = null;

function isNativeUpdaterEnabled(): boolean {
  return app.isPackaged && process.env.CLAWX_DISABLE_UPDATER !== '1';
}

function getAutoUpdater(): ElectronUpdater {
  if (!cachedAutoUpdater) {
    cachedAutoUpdater = require('electron-updater').autoUpdater as ElectronUpdater;
  }
...
export class AppUpdater extends EventEmitter {
  ...
  constructor() {
    ...
    if (!isNativeUpdaterEnabled()) {
      logger.info('[Updater] Native updater disabled in development/unpackaged mode');
      return;
    }
  ...
  async downloadUpdate(): Promise<void> {
    if (!isNativeUpdaterEnabled()) {
      throw new Error('Update download skipped (dev mode - app is not packaged)');
    }
  ...
  quitAndInstall(): void {
    if (!isNativeUpdaterEnabled()) {
      logger.info('[Updater] quitAndInstall skipped in development/unpackaged mode');
      return;
    }
   ...
   *
   * canvasland uses a prompt-first update flow: finding an update shows a UI prompt,
