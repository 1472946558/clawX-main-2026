 ...
 * replacement in that inner process, so the old canvasland.exe can remain alive,
...
const PATCH_MARKER = 'canvasland-patched-v2: run app-running guard in assisted UAC inner instance';
...
const PATCHED_ASSISTED_CHECK = [
  ...
  '  ; UAC inner instance; skipping CHECK_APP_RUNNING there can leave canvasland.exe',
