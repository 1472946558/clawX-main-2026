...
import { ensurecanvaslandContext } from '../utils/openclaw-workspace';
...
export function createAgentsApi(ctx: AgentsApiContext): CompleteHostServiceRegistry['agents'] {
  return {
    ...
    updateProfile: async (payload) => {
      const agentId = requireString(payload, 'id');
      const name = requireString(payload, 'name');
      const persona = requireString(payload, 'persona');
      const snapshot = await updateAgentProfile(agentId, { name, persona });
      scheduleGatewayReload(ctx, 'update-agent-profile');
      return { success: true, ...snapshot };
    },
